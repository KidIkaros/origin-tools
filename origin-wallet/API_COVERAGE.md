...
| Chat | `subscribe`, `publish` | `chat send` (direct / relayed / upgraded / chained tiers), `chat repl` (long-lived node; `--via-relay` publishes + keeps fresh the endpoint's chain-capable hint, RELAY.md §13.7), `chat listen` (+ `--via-relay` publishes the endpoint's chain-capable hint, RELAY.md §13.7) |
