...
| Relay | `dial_relayed`, `dial_relayed_chain_v`, `dial_any`, `upgrade_to_direct`, `serve_relay_with`, `publish_relay_hint_chain`, `publish_relay_hints`, `resolve_relay_hints`, `chain_relays_for`, `chain_relay_for`, `report_relay_peering` | `chat send --relay` (session pipe, upgrade to direct), `chat send --chain` (multi-relay pipe, no single relay learns both endpoints), `chat send --chain-auto` (near relay given, far relay resolved from the peer's chain-capable hint set — tries each fallback in turn), `relay serve` (serves + publishes its chain-capable `chain: true` hint), `relay serve --discovery-point` (R7 relay mesh: register in `stoa:relays`, auto-peer + pre-warm), `relay serve --discovery-refresh` (lower the auto-peer cadence for automation/tests) |
| Payments | `open_channel`, `stream_payment`, `channel_state` | `pay` (direct) and `pay --relay` (A→relay→C), MMR history recording; `pay --cap <n>` (per-tx SpendPolicy narrowing — over-cap refused before anything is sent, tested); `policy` (standing per-tx/day/month caps, persisted with the wallet, enforced before anything binds/signs — tested across save/load) |
| Mail | `send_mail`, `poll_mail`, `mailbox`, `sync_registries` | `mail send`, `mail inbox` (persisted deduped mailbox, reloads across restart) |
| Chat | `subscribe`, `publish`, `accept_relayed` | `chat send` (direct / relayed / upgraded / chained tiers), `chat repl` (long-lived node; `--via-relay` publishes + keeps fresh the endpoint's chain-capable hint, RELAY.md §13.7; accepts relayed/chain circuits and prints them inline), `chat listen` (+ `--via-relay` publishes the endpoint's chain-capable hint, RELAY.md §13.7; accepts relayed/chain circuits so a circuit — not just the addressed topic — terminates here; `--addr` pins the bind for automation) |
