...
| Relay | `dial_relayed`, `dial_relayed_chain_v`, `dial_any`, `upgrade_to_direct`, `serve_relay_with`, `publish_relay_hint_chain`, `chain_relay_for` | `chat send --relay` (session pipe, upgrade to direct), `chat send --chain` (multi-relay pipe, no single relay learns both endpoints), `chat send --chain-auto` (near relay given, far relay resolved from the peer's chain-capable hint), `relay serve` (serves + publishes its hint) |
...
| Chat | `subscribe`, `publish` | `chat send` (direct / relayed / upgraded / chained tiers), `chat repl` (long-lived node), `chat listen` |
